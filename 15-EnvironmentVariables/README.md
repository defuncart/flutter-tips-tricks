...
In `ios/Runner/AppDelegate.swift` (or applicable), parse the dart defines and inject as native iOS env vars

```
// dart defines are not passed by default to the native layer
var secretsDict = [String:String]()
let dartDefinesString = Bundle.main.infoDictionary!["DART_DEFINES"] as! String
for definedValue in dartDefinesString.components(separatedBy: ",") {
    let decoded = String(data: Data(base64Encoded: definedValue)!, encoding: .utf8)!
    let values = decoded.components(separatedBy: "=")
    secretsDict[values[0]] = values[1]
}
MyService.provideAPIKey(secretsDict["MY_KEY"]!)
```
